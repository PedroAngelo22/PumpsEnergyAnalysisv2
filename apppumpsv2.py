...
with st.sidebar:
    ...
    # Inicializa variáveis para garantir que existam em ambos os escopos
    h_man_total = 0
    h_man_manual = 0
    diam_tub = 100.0 
    h_geometrica = 0
...
with col1:
    ...
    if tipo_calculo_h == "Calcular a partir da tubulação":
        ...
        c1.metric("Altura Total", f"{h_man_total:.2f} m", delta="Calculado")
    ...
    st.subheader("Potências e Custos para o Diâmetro Informado")
    ...
    if tipo_calculo_h == "Calcular a partir da tubulação":
        ...
        st.markdown("Este gráfico mostra como o custo energético diminui com o aumento do diâmetro da tubulação, devido à menor perda de carga.")
        
        # Cria um dicionário com todos os parâmetros que a função do gráfico precisa.
        params_grafico = {
            "vazao": vazao,
            "h_geometrica": h_geometrica,
            "comp_tub": comp_tub,
            "rug_tub": rug_tub,
            "k_total_acessorios": k_total_acessorios,
            "rend_bomba": rend_bomba / 100,
            "rend_motor": rend_motor / 100,
            "horas_por_dia": horas_por_dia,
            "tarifa_energia": tarifa_energia,
            "fluido_selecionado": fluido_selecionado
        }
        
        # Chama a função desempacotando o dicionário.
        chart_data = gerar_grafico_diametro_custo(**params_grafico)
        st.bar_chart(chart_data.set_index('Diâmetro da Tubulação (mm)'))
...
with col2:
    ...
    # Coleta os dados para o relatório em um dicionário
    inputs_relatorio = {
